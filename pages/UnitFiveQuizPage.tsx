...
const UnitFiveQuizPage: React.FC = () => {
  const { t } = useAppContext();
  const questions: QuizQuestion[] = useMemo(() => t('unit5.quiz.questions'), [t]);
            ...
            return (
              <div key={index} className={`p-6 rounded-lg shadow-md ${isCorrect ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'} border`}>
                ...
                <p className="text-sm text-gray-600">
                  {t('unit5.quiz.yourAnswer')} <span className={`font-semibold ${isCorrect ? 'text-green-700' : 'text-red-700'}`}>{userAnswer !== null ? q.options[userAnswer] : 'No answer'}</span>
                </p>
                {!isCorrect && (
                  <p className="text-sm text-gray-600 mt-1">
                    {t('unit5.quiz.correctAnswer')} <span className="font-semibold text-green-700">{q.options[q.correctAnswer]}</span>
                  </p>
                )}
              </div>
            );
  ...
  return (
    <div>
      <SectionTitle
        title={t('unit5.quiz.title')}
        subtitle={t('unit5.quiz.subtitle')}
      />
      ...
      <div className="max-w-4xl mx-auto bg-white p-8 rounded-lg shadow-xl">
        <div className="flex justify-between items-center mb-6">
          ...
          <span className="text-sm font-semibold text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
            {t('unit5.quiz.question', { current: currentQuestionIndex + 1, total: questions.length })}
          </span>
        </div>
...
export default UnitFiveQuizPage;